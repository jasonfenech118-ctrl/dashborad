...
def roster(request):
    ...
    # Single timeline: upcoming/today first (soonest next), then past history (most recent first).
    ...
    for e in ot_upcoming:
        e.status = "today" if e.date == today else "upcoming"
    for e in ot_past:
        e.status = "past"
    ot_timeline = ot_upcoming + ot_past
